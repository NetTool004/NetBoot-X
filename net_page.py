...
def build_net_page(parent, app):
    ...
    labels = [app.t("net_dns"), app.t("net_tcp"), app.t("net_flush")]
    ...
    current = app.t("status_on") if app.app_state["net_enabled"] else app.t("status_off")
    status = ctk.CTkLabel(
        box,
        text=f"{app.t('net')}: {current}",
    ...
    action_text = app.t("disable") if app.app_state["net_enabled"] else app.t("apply")
