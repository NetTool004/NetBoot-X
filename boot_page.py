...
def build_boot_page(parent, app):
    ...
    labels = [app.t("boot_fast"), app.t("boot_delay"), app.t("boot_timeout")]
    ...
    current = app.t("status_on") if app.app_state["boot_enabled"] else app.t("status_off")
    status = ctk.CTkLabel(
        box,
        text=f"{app.t('boot')}: {current}",
    ...
    action_text = app.t("disable") if app.app_state["boot_enabled"] else app.t("apply")
